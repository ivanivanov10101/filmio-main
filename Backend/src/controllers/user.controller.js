...
export const updateUser = asyncHandler(async (req, res, next) => {
  ...
  if (req.user.id !== req.params.userId) {
    return next(
      new customError(403, "You are not allowed to update this user"),
    );
  }
  ...
  if (userName) {
    if (userName.length < 7 || userName.length > 20) {
      return next(
        new customError(400, "userName must be at between 7 and 20 characters"),
      );
    }
    ...
    if (!userName.match(/^[a-zA-Z0-9]+$/)) {
      return next(
        new customError(400, "userName can only contains letter or number"),
      );
    }
    const isUsernameMatched = await Account.findOne({
  ...
  if (password) {
    if (password.length < 6) {
      return next(
        new customError(400, "Password must be at least 6 characters"),
      );
    }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { updatedUser }, "user successfully updated"));
});

export const deleteUser = asyncHandler(async (req, res, next) => {
  if (!req.user.isAdmin && req.user.id !== req.params.userId) {
    return next(
      new customError(403, "You are not allowed to delete this user"),
    );
  }
...
export const logoutUser = asyncHandler(async (req, res) => {
  if (req.user.id !== req.params.userId) {
    return next(
      new customError(403, "You are not allowed to logout this user"),
    );
  }
...
export const getUser = asyncHandler(async (req, res, next) => {
  const user = await Account.findById(req.params.userId).select(
