...
export const deletePost = asyncHandler(async (req, res, next) => {
  if (req.user.id !== req.params.userId) {
    return next(
      new customError(403, "You are not allowed to delete this post"),
    );
  }
...
export const updatePost = asyncHandler(async (req, res, next) => {
  if (req.user.id !== req.params.userId) {
    return next(
      new customError(403, "You are not allowed to update this post"),
    );
  }
  ...
  res
    .status(201)
    .json(
      new ApiResponse(201, updatedPost, "post has been updated successfully"),
    );
});
